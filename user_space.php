...
// Menentukan URL saat ini agar pengalihan (redirect) tetap berada di halaman yang sama (My Work / User Space)
...
if (isset($_POST['toggle_verif']) && $_SESSION['Role'] == 'admin') {
    ...
    if ($target_role !== 'admin') {
        ...
        if ($stmt_update->execute()) {
            echo "<script>alert('Status Verifikasi Berhasil Diubah!'); window.location.href='$current_page_url';</script>";
        }
    } else {
...
// PERBAIKAN LOGIKA: Jika $_GET['UserID'] kosong (diakses via My Work), otomatis pakai $_SESSION['UserID']
    ...
    <div class="mb-16">
        ...
        <form action="config/proses_hapus_album.php" method="POST">
            ...
            <div class="flex flex-wrap justify-center gap-4">
                <a href="<?= isset($_GET['UserID']) ? 'user_space.php?UserID='.$target_user_id : 'mywork.php' ?>"
                ...
                <?php while ($alb = $query_album_list->fetch_assoc()): ?>
                    <div class="relative group">
                        ...
                        <a href="<?= isset($_GET['UserID']) ? 'user_space.php?UserID='.$target_user_id.'&album='.$alb['AlbumID'] : 'mywork.php?album='.$alb['AlbumID'] ?>"
...
<script>
    window.activateRemoveMode = function() {
        ...
        checkbox.forEach(el => el.classList.toggle('hidden'));
    };
</script>
